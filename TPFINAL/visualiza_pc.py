...
#plt.title("Frecuencia: T=%i Hz" %t)
fig    = plt.figure ( 1 )

adcAxe = fig.add_subplot ( 2,1,1                            )
adcLn, = plt.plot        ( [],[],'r-',linewidth=4   ,label = "adc"        )
adcAxe.grid              ( True                             )
adcAxe.set_ylim          ( -2 ,2                            )
adcLg      = adcAxe.legend()

fftAxe      = fig.add_subplot ( 2,1,2                                 )
fftLn,      = plt.plot        ( [],[],'b-',linewidth = 5,alpha  = 1 ,label = "FFT(adc)" )
ciaaFftLn,  = plt.plot        ( [],[],'r-',linewidth = 5,alpha = 0.4  ,label = "CiaaFFT" )
